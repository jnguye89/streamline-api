...
export class AgoraStreamRepository {
  constructor(
    @InjectRepository(AgoraStream)
    private readonly agoraStreamRepo: Repository<AgoraStream>,
  ) {}
  ...
  async findByChannelName(channelName: string): Promise<AgoraStream | null> {
    return this.agoraStreamRepo.findOne({ where: { channelName } });
  }
  ...
  async createNew(channelName: string, userId: string): Promise<AgoraStream> {
    ...
    return this.agoraStreamRepo.save(newStream);
  }
  ...
  async updateStatus(
    ...
    return this.agoraStreamRepo.save(stream);
  }
  ...
  async heartbeat(channelName: string): Promise<void> {
