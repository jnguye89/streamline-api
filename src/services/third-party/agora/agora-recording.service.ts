...
export class AgoraRecordingService {
  ...
  async getResourceId(channelName: string, userId: string) {
    ...
    const { data } = await firstValueFrom(
      this.http.post<{ resourceId: string }>(url, payload, {
        headers: {
  ...
  async startRecording(channelName: string) {
    ...
    const token = await this.agoraTokenService.createTokens(
      podcast.recordingUid,
      podcast.channelName,
    );
    const payload = {
      cname: channelName,
      ...
      clientRequest: {
        ...
        storageConfig: {
          vendor: 1, // 1 = AWS S3
          region: Number(process.env.AGORA_S3_REGION ?? 2),
    ...
    const { data } = await firstValueFrom(
      this.http.post<{ sid: string }>(url, payload, {
        headers: {
          Authorization: `Basic ${this.agoraTokenService.createBasicAuthToken()}`,
        },
      }),
    ...
    const queryUrl = `${this.baseUrl}/${process.env.AGORA_APP_ID}/cloud_recording/resourceid/${encodeURIComponent(podcast.resourceId)}/sid/${encodeURIComponent(data.sid)}/mode/mix/query`;
  ...
  async stopRecording(channelName: string): Promise<string | undefined> {
    ...
    const { data } = await firstValueFrom(
      this.http.post<AgoraStopResponseDto>(url, payload, {
        headers: {
          Authorization: `Basic ${this.agoraTokenService.createBasicAuthToken()}`,
        },
      }),
    ...
    const videoPath = data.serverResponse.fileList.find((f) =>
      f.fileName.endsWith('.mp4'),
    )?.fileName;

    if (!videoPath) {
      console.error(
        'No MP4 file found in Agora response:',
