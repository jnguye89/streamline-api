...
export class WowzaService {
  ...
  public async createStream(
    userId: string,
    broadcastLocation: string = 'us_west_oregon',
    ...
    payload.live_stream.broadcast_location = broadcastLocation;

    try {
      const { data } = await firstValueFrom(
        this.http.post<LiveStreamResponse>(
          `${this.wowzaUrl}/live_streams`,
          payload,
          {
      ...
      const e = error as AxiosError<WowzaErrorResponse>;
  ...
  async startLiveStream(wowzaId: string) {
    await firstValueFrom(
      this.http.put(`${this.wowzaUrl}/live_streams/${wowzaId}/start`, null, {
        headers: { Authorization: `Bearer ${this.wowzaToken}` },
      }),
    );
  }
  ...
  async stopLiveStream(wowzaId: string) {
    await firstValueFrom(
      this.http.put(`${this.wowzaUrl}/live_streams/${wowzaId}/stop`, null, {
        headers: { Authorization: `Bearer ${this.wowzaToken}` },
      }),
    );
  }
  ...
  async getLiveStream(wowzaId: string): Promise<LiveStreamResponse> {
    const res = await firstValueFrom(
      this.http.get(`${this.wowzaUrl}/live_streams/${wowzaId}`, {
        headers: { Authorization: `Bearer ${this.wowzaToken}` },
      }),
    );
    return res.data;
  ...
  async isReadyState(ls: LiveStreamResponse): Promise<boolean> {
    ...
    if (isActive) {
      const { data } = await firstValueFrom(
        this.http.get<StreamStatusDto>(
          `${this.wowzaUrl}/analytics/ingest/live_streams/${ls.live_stream.id}`,
          {
            headers: { Authorization: `Bearer ${this.wowzaToken}` },
          },
        ),
  ...
  async isStreaming(streamId: string): Promise<boolean> {
    const { data } = await firstValueFrom(
      this.http.get<StreamStatusDto>(
        `${this.wowzaUrl}/analytics/ingest/live_streams/${streamId}`,
        {
          headers: { Authorization: `Bearer ${this.wowzaToken}` },
        },
      ),
  ...
  async getRecording(
    recordingId: string,
  ): Promise<{ download_url: string; video_id: string; extension: string }> {
    const { data } = await firstValueFrom(
      this.http.get<WowzaVideoDto>(`${this.wowzaUrl}/videos/${recordingId}`, {
        headers: { Authorization: `Bearer ${this.wowzaToken}` },
      }),
    );

    const video = data.video.encodings
      .filter((e) => e.video_container == 'mp4')
      .sort((a, b) => (a < b ? -1 : a > b ? 1 : 0))[0];
    if (!video.video_file_url) throw new NotFoundException('Missing vide url');
    return {
      download_url: video.video_file_url,
      video_id: data.video.id,
      extension: video.video_container,
    };
  }

  async deleteRecording(recordingId: string): Promise<void> {
    await firstValueFrom(
      this.http.delete(`${this.wowzaUrl}/videos/${recordingId}`, {
        headers: { Authorization: `Bearer ${this.wowzaToken}` },
      }),
    );
  }
