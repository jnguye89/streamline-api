...
export class StreamService {
  constructor(
    ...
    private wowza: WowzaService,
    private events: StreamsEvents,
  ...
  async ensureReady(broadcastLocation: string, user: string): Promise<Stream> {
    ...
    if (!stream) {
      const streamDto = await this.wowza.createStream(user, broadcastLocation);
      const userDto = await this.userService.getUser(user);
      stream = await this.repo.save({
        wowzaId: streamDto.live_stream.id,
        broadcastLocation: streamDto.live_stream.broadcast_location,
        user: { auth0UserId: userDto.auth0UserId },
        phase: 'idle',
        wssStreamUrl:
          streamDto.live_stream.source_connection_information?.sdp_url,
        applicationName:
          streamDto.live_stream.source_connection_information?.application_name,
        streamName:
          streamDto.live_stream.source_connection_information?.stream_name,
      });
    }
    ...
    if (!isReadyState) {
      this.startAndPoll(stream.id, stream.wowzaId).catch(
        async (error: unknown) => {
          await this.setPhase(
            stream.id,
            'error',
  ...
  private async startAndPoll(id: number, wowzaId: string) {
    ...
    await this.setPhase(
      ...
      'error',
      undefined,
      'Timed out waiting for Wowza to start.',
    );
    // await this.repo.update({ id }, { isProvisioning: false });
  }
